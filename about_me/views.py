...
def search_topic(request,pk):
	...
	result = ftopics
	search_value = request.POST.get('saerch_val')
	if  search_value and  request.method == 'POST':
		qs =  list(FrontEndTopics.objects.filter(Q(course_name__exact=pk) & Q(topic_name__icontains = search_value)).values())
		if  len(qs) >0:
			print(qs)
			result = qs
		...
		qs =  FrontEndTopics.objects.filter(course_name__exact=pk)
		result=qs
		#return  JsonResponse(result, safe=False)
		return render(request, 'about_me/frontendcourse_detail.html',{'course':course,'ftopics':result})
